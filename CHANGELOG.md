# Changelog

All notable changes to this project will be documented in this file.

The format is inspired by **Keep a Changelog**, and the project follows semantic versioning where appropriate.

---

## [Unreleased]

### Added

* Created Verdis Flutter project.
* Configured GitHub repository and Git version control.
* Configured Flutter development environment.
* Established feature-first project architecture.
* Created project documentation structure.
* Added sprint planning documentation.
* Added meeting notes and ADR structure.
* Organized project assets directory.
* Replaced the default Flutter counter application.
* Implemented the initial MaterialApp and Scaffold.
* Created the first custom Verdis application screen.

### Documentation

* Documented Sprint 1.
* Documented project setup decisions.
* Defined initial project architecture.

### Learning & Architecture

* Studied Flutter widget hierarchy.
* Learned StatelessWidget fundamentals.
* Learned MaterialApp and Scaffold responsibilities.
* Learned BuildContext and widget tree concepts.
* Established development workflow using Git and Agile practices.
